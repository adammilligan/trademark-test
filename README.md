...
Vite поднимает приложение на `http://localhost:5173`.
...
Runtime‑состояние (таймер, буфер, генератор слов) вынесено за пределы React‑состояния, чтобы не нагружать рендер постоянными апдейтами и сохранять плавный 60 FPS (Проверено!).
